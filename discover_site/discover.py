...
def main():
    ...
    # NEW: Save site config for downstream modules (e.g., RAG pipeline)
    parsed_url = urlparse(args.url)
    site_key = parsed_url.netloc.replace(".", "-")
    config = {
        "base_url": args.url,
        "domain": parsed_url.netloc,
        "site_key": site_key  # e.g., cyberizegroup-com
    }

    with open("site_config.json", "w") as f:
        json.dump(config, f, indent=2)
    print(f"[INFO] Saved config to /site_config.json")
